import 'package:flutter/material.dart';

class AppColors {
  // Основные цвета
  static const Color bgMain = Color(0xFF010101);
  static const Color bgSurface = Color(0xFF1F1F1F);

  // Акцентные цвета
  static const Color accentPrimary = Color(0xFF9F32FF);
  static const Color accentSecondary = Color(0xFFC738D8);

  static const Color gradientDark = Color(0xFF6900C5);
  static const Color gradientLight = Color(0xFFA845FF);

  static const Color gradientBgSurface = Color(0xFF373737);

  // Цвета текста
  static const Color textPrimary = Color(0xFFEAEAEF);
  static const Color textSecondary = Color(0xFFA9A9A9);
  static const Color textDisabled = Color(0xFF606060);

  static const Color navBarLine = Color(0xFF404050);

  // Семантические цвета (статусы)
  static const Color greenColor = Color(0xFF28A745);
  static const Color yellowColor = Color(0xFFFFC107);
  static const Color redColor = Color(0xFFDC3545);
  static const Color blueColor = Color(0xFF00A3FF);
  static const Color greyColor = Color(0xFFA9A9A9);
  static const Color medalBronza = Color(0xFFC05A00);
}
